...
- **Mantine** escolhida (não MUI/Chakra) como lib de componentes; **`mantine-datatable`** para grids
  (default, **estável** no Mantine 7). **`mantine-react-table` (MRT)** é a "parruda" mas **no Mantine 7
  só existe em beta** → **fica de fora** pela regra [[so-dependencia-estavel]] (só estável, nada de beta);
  reavaliar quando sair 2.x estável. **Datas: sempre `DatePickerInput` (`@mantine/dates`, pt-BR)** —
  nunca `<input type="date">` nativo (foi o "amador" do 1º piloto por faltar `@mantine/dates` no
  scaffold; corrigido).
