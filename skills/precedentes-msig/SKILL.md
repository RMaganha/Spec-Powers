...
| Ambiente/infra corporativa (rede, proxy, Postgres, SQL Server, Azure pipeline) | vários — ver `modelo/AMBIENTE.md.modelo` do kit Spec-Powers (`C:\Ronaldo\_Mitsui\Python\Spec-Powers`) | Fatos fixos + padrões já consolidados num único doc de referência | Use esse arquivo diretamente em vez de reinvestigar do zero |
