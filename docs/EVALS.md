...
- **Reincidência (2026-09-22):** commit `f9388e5` (Task 13 do inventário do banco) saiu de `python -m pytest -q 2>&1 | tail -1 && git add … && git commit`. A suíte estava verde (428), então não houve dano — mas a memória **não bastou**, e a contagem de "reincidência zero em 3 sessões" recomeça. Detalhe que importa pro guardrail: na mesma sessão, os subagentes receberam a regra **por escrito no prompt** e a seguiram em todos os commits; quem reincidiu foi o controlador, ao trocar pra execução direta pra ganhar tempo. Prosa na memória não chega no momento do atalho → reforça o caminho da **cerca** (hook que nega `pytest … | … && git commit` no mesmo comando).
