...
- [check de versão do plugin contra o remoto](specs/2026-07-21-doctor-check-versao-remoto-design.md) — novo check no `/mss-spec:doctor`: compara a versão instalada do kit com a publicada no remoto (via `git fetch` no clone, semver) e reporta ✓/⚠/ℹ, só reportando; degrada gracioso offline/dev — **implementada** (feature/doctor-check-versao, suíte 63 verde; aguarda release+merge)
