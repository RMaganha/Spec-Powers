...
## Onde estamos
`main` — **0.23.0 mergeada** (`2908197`, merge `--no-ff` de `feature/diagnostico`; suíte **160 verde**; INDEX fechado no histórico). **Trilho disciplinado de diagnóstico (F-015)**, nascido da análise do PDF da sessão de deploy do MSS-SSC (6 rodadas de owner num 503 + 2 re-litigando App Setting correta). 4 camadas: regra crítica 11 no molde (sempre-ativa; o CLAUDE.md ficou em 7.938/8.000 bytes via compressão mover-não-apagar) · `/mss-spec:diagnostico` (alavanca do owner) · memória `feedback_diagnostico_disciplinado` + F-015 no EVALS · propagação via `upgrade` (mescla + renumera). Spec: `docs/specs/diagnostico.md`.
