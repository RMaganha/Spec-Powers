...
## Próximo passo
**Integrar** a v0.14.0: revisar o diff, merge `--no-ff` de `feature/analise-projeto-existente` na `main` e — quando o owner pedir — `git push` (a `main` está **7 commits à frente** do `origin/main`; sem push, os outros projetos não pegam o `analise` nem com `claude plugin marketplace update`). Primeiro uso real previsto: rodar o `/mss-spec:analise` no projeto de **RAG/pgvector** do owner. No `to-dolist`, aberto: perguntar na constituição se o projeto segue a **arquitetura de rede MSIG** ou tem **arquitetura própria** (hoje o kit assume MSIG).
