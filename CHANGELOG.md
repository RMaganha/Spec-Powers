...
- build tools: **vite 8 + `@vitejs/plugin-react` 6** (peer `vite ^8`; limpam a vuln dev-only) e **TypeScript no 5.x maduro** — o TS 7 (reescrita recém-lançada do compilador) fica de fora por ora: não é beta, mas é bleeding-edge. Régua: runtime = mais novo estável; major de ferramenta recém-nascido = esperar assentar
