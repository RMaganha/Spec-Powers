...
2. **SQL Server — padrão canônico da casa: `get_connection.py` multi-ambiente** (referência real: `C:\Ronaldo\_Mitsui\Python\Transportes\V2\get_connection.py`):
   ...
   - `.env.example` recebe só dois seletores, **comentários em linha própria** (nada inline — o Docker Compose `env_file` passa o comentário junto do valor e quebra a conexão): `CONEXAO_SQL=D0` (D0|HML|PRD) e `CONEXAO_SQL_PORTA=` (vazio = porta padrão do SQL; preenchida sobrescreve).
