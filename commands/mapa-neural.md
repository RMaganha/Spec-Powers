...
2. **Rode no projeto atual:** `python "${CLAUDE_PLUGIN_ROOT}/templates/mapa_neural.py"` (opcional: `--proj <dir> --out <dir>`; `--ignorar pasta1,pasta2` tira pastas versionadas que só fazem ruído; `--limite N` muda o teto por ramo, default 200). As saídas caem em **`docs/`** por padrão (toda pasta do superpowers tem `docs/`, fica isolado da raiz; ambas gitignoradas — derivadas/regeneráveis).
