...
7. **Corpus de falhas em dia** (se existe `docs/EVALS.md`) — conte os casos com status **`aberto`** (falha registrada que ainda **não** virou guardrail) e liste-os por id + gatilho. **⚠ que informa, não trava** o release: é dívida de prosa, não defeito de código. Sinalize ✗ só num caso: a entrega desta branch **derrubou uma premissa** ou repetiu uma falha e **nada** foi registrado no corpus — aí o `/mss-spec:memory capturar` não rodou.
8. **Working tree** (se é repo git) — sem mudança relevante perdida (não-commitada) e **`.env`/segredo fora do stage** (`git status`); nunca ecoe valor de segredo, só sinalize o risco.
