...
2. **Traga do plugin — doc SEMPRE, scaffold só na 1ª vez:**
   - **`docs/FRONTEND.md`: (re)sincronize SEMPRE** a partir de `${CLAUDE_PLUGIN_ROOT}/templates/FRONTEND.md`, a cada vez que este comando rodar. Assim o projeto pega as melhorias acumuladas do molde (busca na grid, `StatusBadge`/`StatusLegend` + regra da legenda, export server-side, notas de versão). É seguro — é doc de referência; só preserve os rótulos do sistema (nome/subtítulo/rodapé) se você os tinha customizado.
   - **Scaffold (`frontend/`): copie só se AINDA NÃO existir.** Se `frontend/` já existe, **NÃO sobrescreva** — os componentes do projeto estão lá; apenas confira se `package.json`/config batem com o padrão atual. Na 1ª vez, copie `${CLAUDE_PLUGIN_ROOT}/templates/frontend/` inteiro (package.json, vite.config.ts, tsconfig.json, index.html, src/main.tsx, src/theme.ts, src/components/, README.md). Tema MSIG em `src/theme.ts` (`brand`/`navy`). Grid = `mantine-datatable` (estável); datas = **`DatePickerInput`** do `@mantine/dates` em pt-BR (nunca `<input type="date">` nativo). **Regra: só dependências estáveis — nada de beta/rc.** A MRT ("parruda") fica de fora enquanto não houver estável pro Mantine atual (ver `docs/FRONTEND.md`).
...
5. **Endpoint de dados:** o React consome **JSON** do FastAPI — crie/aponte o endpoint dedicado (a camada Python vira API/BFF). Nunca fazer o React ler HTML renderizado. **Busca na grid** = filtro client-side p/ poucos registros, ou `?busca=` no endpoint p/ muitos. **Exportar Excel** = SEMPRE no servidor (FastAPI + `openpyxl`), nunca lib de export no front (ver `docs/FRONTEND.md`).
