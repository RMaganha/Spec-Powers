...
- **Git local-only**: commits são locais e com stage **nominal** (`git add <caminhos>`; nunca `git add .`/`-A`, que varreria `.env`/segredos). **`git push` só quando eu pedir explicitamente.**
