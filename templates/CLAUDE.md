<!-- MODELO de CLAUDE.md — copie para a RAIZ do projeto e preencha os <...>. ENXUTO: entra na janela
     em TODA sessão, e inchado faz a regra que importa se perder. Teste de cada linha: "removendo isto,
     ele erraria?". Procedimento longo vai pro comando, pra `.claude/rules/` ou pra spec do assunto. -->
...
- **Comando `/mss-spec:<x>` citado EXISTE — leia o arquivo `commands/<x>.md`**: é `disable-model-invocation`, por isso não aparece na sua lista de invocáveis. Nunca conclua "não existe" nem chute o conteúdo. **"Rode `/mss-spec:X`" = execute os passos do arquivo na mão** — invocar falha com *"Falha ao executar a habilidade"*. Kit parecendo ausente é diagnóstico do `/mss-spec:doctor`.
...
- **Na partida, leia nesta ordem**: `docs/superpowers/MAPA.md` (onde estamos · próximo passo) → `memory/MEMORY.md` (índice por **gatilho**: cada linha diz *quando* abrir aquela memória) → `docs/superpowers/INDEX.md` (tarefas abertas) → `docs/EVALS.md` (falhas que já custaram caro — leia a coluna gatilho). São **índices**: abra o arquivo individual só quando apontarem relevância, nunca a pasta inteira.
...
## Contexto
- **Infra:** <**MSIG** | **própria**> — MSIG = rede `mitiai_network`, proxy, CA do FortiGate, SQL Server corporativo (`docs/AMBIENTE.md`). Própria = nada disso se aplica (sem proxy no `.env`, sem `docker-compose.office.yml`, sem o `get_connection.py` corporativo) e o `/mss-spec:doctor` pula esses checks. Definido no `/mss-spec:kickoff`; é esta linha que os comandos leem.
...
8. **O projeto ativo é a ÂNCORA; outro projeto é SOMENTE-LEITURA.** A âncora é a raiz onde a janela abriu e **não migra**. Lá só se lê: `Write`/`Edit`, `git`, `pytest` ou "consertar de passagem", **nunca** — traga o padrão pra cá. Mudança é lá? **pare e diga** que é janela nova na raiz dele; bug visto lá, **reporte**. Nasceu de acidente real (o projeto B foi alterado e quebrou). Cerca: hook `PreToolUse` (escape consciente `MSS_ANCORA_OFF=1`) — mas cerca é 2ª linha, a regra é esta.
...
11. <regra específica do seu projeto…>

<!-- Cresceu uma seção? Mova o detalhe pro comando, pra `.claude/rules/` ou pra spec, e deixe o ponteiro.
     Teto deste arquivo: 8 KB — o `/mss-spec:doctor` mede e avisa. -->
