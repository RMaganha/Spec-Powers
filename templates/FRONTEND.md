...
- **Build tools:** **vite 8 + `@vitejs/plugin-react` 6** (andam juntos — plugin-react 6 tem peer `vite ^8`; limpam uma vuln que é **só do dev server**, não do bundle). **TypeScript no 5.x maduro** — o **TS 7 é reescrita recém-lançada do compilador**: não é beta, mas é bleeding-edge (ecossistema/editor ainda assentando) → evitar até estabilizar. Mesma régua do "só estável": runtime = mais novo estável; major de ferramenta recém-nascido = esperar assentar.
