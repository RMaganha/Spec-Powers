...
- **Build tools no último estável:** **vite 8 + `@vitejs/plugin-react` 6** (peer `vite ^8`; limpam uma vuln que é **só do dev server**, não do bundle) **+ TypeScript 7**. Detalhe que torna isso seguro: o **build (vite/esbuild) não usa o `tsc`** — o TypeScript roda só no `npm run typecheck`. Então o `typecheck` é o juiz: passou, você está no mais novo; reclamou, **caia o `typescript` pra `^5.6`** (uma linha) e siga. Nada de beta/rc — mas major novo estável entra, com o typecheck de rede de segurança.
- **Números exatos rot** (Mantine 7→9, React 18→19, vite 6→8 sem a gente ver): o scaffold pina o estável do dia; no install, **confira o `latest` no npm** e rode `npm run typecheck`.
- **Fonte da verdade das versões:** o template oficial **`mantinedev/vite-template`** (GitHub). Este scaffold bate com ele (verificado 2026-07-13: Mantine 9.4.1, React 19.2, vite 8, plugin-react 6, **TypeScript 7.0.2**). Em dúvida de versão, abra o `package.json` dele — não chute de memória.
